...
use net_traits::request::{
    CacheMode, CredentialsMode, Destination, Origin, PreloadedResources, RedirectMode, Referrer,
    RequestBuilder, RequestClient, RequestMode, ServiceWorkersMode,
};
use net_traits::{
...
use crate::dom::bindings::codegen::Bindings::WebTransportBinding::{
    WebTransportCongestionControl, WebTransportMethods, WebTransportOptions,
};
...
struct WebTransportFetchListener {}

impl FetchResponseListener for WebTransportFetchListener {
    fn process_request_body(&mut self, _request_id: RequestId) {}

    /// Step 34 of <https://w3c.github.io/webtransport/#webtransport-constructor>
    fn process_response(
        &mut self,
        _cx: &mut JSContext,
        _request_id: RequestId,
        _metadata: Result<FetchMetadata, NetworkError>,
    ) {
        // TODO:
        // Step 34.1. Process a WebTransport fetch response with response and transport.
    }

    fn process_response_chunk(&mut self, _cx: &mut JSContext, _request_id: RequestId, _chunk: Vec<u8>) {}

    fn process_response_eof(
        self,
        _cx: &mut JSContext,
        _request_id: RequestId,
        _response: Result<(), NetworkError>,
        _timing: ResourceFetchTiming,
    ) {}
