...
use x_wing::{
    DecapsulationKey as MlKem768X25519DecapsulationKey, Decapsulator,
    EncapsulationKey as MlKem768X25519EncapsulationKey, KeyExport, KeyInit, TryKeyInit,
};
        ...
        KeyFormat::Raw_public => {
            ...
            let public_key = match normalized_algorithm.name {
                ...
                CryptoAlgorithm::MlKem768X25519 => {
                    ...
                    let encapsulation_key = MlKem768X25519EncapsulationKey::new_from_slice(data)
                        .map_err(|_| {
                            Error::Data(Some(
